    ...
    <div className="min-h-screen bg-background flex items-center justify-center p-4">
      <div className="bg-card rounded-2xl border border-border shadow-sm p-8 w-full max-w-md animate-fade-in">
        <div className="text-center mb-6">
          ...
          <h1 className="text-xl font-bold text-foreground mt-2">Chào mừng trở lại</h1>
        </div>
        ...
        <form onSubmit={(e) => { e.preventDefault(); navigate("/dashboard"); }} className="space-y-4">
          ...
          <div>
            ...
            <div className="relative mt-1">
              <input type={showPw ? "text" : "password"} required className="w-full px-4 py-3 rounded-xl border border-input bg-background text-foreground text-sm focus:outline-none focus:ring-2 focus:ring-ring pr-12" />
          ...
          <Link to="/forgot-password" className="text-sm text-primary font-medium">Quên mật khẩu?</Link>
          <button type="submit" className="w-full bg-primary text-primary-foreground py-3 rounded-xl font-semibold hover:opacity-90 transition-opacity min-h-[44px]">Đăng nhập</button>
        </form>

        <p className="mt-6 text-center text-sm text-muted-foreground">
          Chưa có tài khoản? <Link to="/register" className="text-primary font-medium">Đăng ký miễn phí</Link>
        </p>
      </div>
    </div>
  );
